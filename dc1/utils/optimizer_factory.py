...
class OptimizerFactory:
    ...
    def get_optimizer(model: nn.Module, config: Dict[str, Any]) -> optim.Optimizer:
        ...
        if not weight_decay_on_bias:
            params = [
                {"params": [p for n, p in model.named_parameters() if not n.endswith("bias")], "weight_decay": weight_decay},
                {"params": [p for n, p in model.named_parameters() if n.endswith("bias")], "weight_decay": 0.0},
            ]
        else:
        ...
        if optimizer_type == 'adam':
            ...
            return optim.RMSprop(params, lr=learning_rate, momentum=momentum, weight_decay=weight_decay)
        else:
